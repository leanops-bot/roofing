...
import { Shield, MapPin, CheckCircle, ArrowRight, Camera, Trophy } from "lucide-react";
import { BookingFormApex } from "@/components/features/BookingFormApex";

export const HeroProjects = ({ city = "Sarasota" }: { city?: string }) => {
    return (
        <section className="relative min-h-screen flex items-center pt-32 pb-24 overflow-hidden bg-primary-navy">
            {/* Cinematic Background Image - Drone Project Visual */}
            <div className="absolute inset-0 z-0">
                <img
                    src="https://images.unsplash.com/photo-1635424710928-0544e8512eae?q=80&w=2000&auto=format&fit=crop"
                    alt="Premium Roofing Projects Florida"
                ...
                <div className="grid lg:grid-cols-2 gap-16 items-center">
                    ...
                    <div className="max-w-3xl">
                        ...
                        >
                            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary-gold-500/10 border border-primary-gold-500/20 mb-8">
                                <Camera className="w-4 h-4 text-primary-gold-500" />
                                <span className="text-primary-gold-500 text-[10px] font-black uppercase tracking-[0.2em]">Curated Project Showcase</span>
                            </div>

                            <h1 className="text-[48px] md:text-[68px] lg:text-[84px] font-black tracking-tight leading-[0.95] text-white mb-8 italic uppercase">
                                <span className="text-primary-blue not-italic underline decoration-primary-gold-500/30 decoration-8 underline-offset-8">Roofing</span> <br />
                                <span className="text-white not-italic">Projects in {city}</span>
                            </h1>

                            <p className="text-xl text-text-muted mb-10 leading-relaxed font-medium max-w-xl">
                                Explore premium roofing transformations completed by Apex Roofing — built for Florida storms, designed for long-term protection.
                            </p>

                            <div className="grid sm:grid-cols-2 gap-x-8 gap-y-4 mb-12">
                                {[
                                    { text: "Premium Completed Projects", icon: Trophy },
                                    { text: "15+ Years Experience", icon: Shield },
                                    { text: "Dedicated Job Foreman", icon: CheckCircle },
                                    { text: "Certified Installations", icon: MapPin },
                                ].map((item, idx) => (
                                    <div key={idx} className="flex items-center gap-3">
                                        <div className="w-5 h-5 rounded-full bg-primary-blue/20 flex items-center justify-center">
                                            <item.icon className="w-3 h-3 text-primary-blue" />
                                        </div>
                    ...
                    <div className="hidden lg:block">
                        ...
                        >
                            ...
                            {/* Pro Badge */}
                            <div className="absolute -right-12 -top-6 hidden xl:flex flex-col items-center justify-center w-32 h-32 bg-primary-blue text-primary-navy rounded-full shadow-2xl z-20 font-black -rotate-6 border-4 border-white animate-pulse">
                                <Camera className="w-8 h-8 mb-1" />
                                <span className="text-[10px] tracking-[0.2em] leading-none mb-1">REAL</span>
                                <span className="text-2xl leading-none">PROOFS</span>
                            </div>
                        </motion.div>
                    </div>
                </div>
